...
const go = async () => {
  ...
  const response = await fetch.default("https://typescript.azureedge.net/indexes/releases.json")
  ...
  const preReleaseResponse = await fetch.default("https://typescript.azureedge.net/indexes/pre-releases.json")
...
`
  const path = join(__dirname, "..", "src", "app", "sandbox", "src", "release_data.ts")
  writeFileSync(path, format(code, { filepath: path }), "utf8")

  const jsonPath = join(__dirname, "..", "src", "app", "sandbox", "src", "releases.json")
